...
- **Task affinity (load-bearing):** the service hook must drive stroke detection **only when invoked on the Arduino `loop()` task**. Audio triggered from a BLE write callback runs on a *separate* FreeRTOS task, and the loop runs concurrently there — so servicing detection from the callback raced `loop()`'s own IMU read and **deadlocked the non-reentrant I2C bus, freezing the firmware** (LED stuck, power button dead, after session end). The hook guards on the captured loop task handle (`g_loopTaskHandle`) and stands down on any other task; loop-context audio (the speed call-out) is the only place it both helps and is safe. Do not remove that guard.
- Verified on-device: detection now runs through prompts (counts advance, no dropped notifies), and the paddle completes a session and powers off normally.
