...
```
External audio ready (session summary prompts on QSPI).
[extAudio] JEDEC 0x...
[extAudio] ready, prompts=12
  0x8  off=0x9C  samples=...
  ...
```

Run a full session from the Android app. When it finishes, the device should
say e.g. "Excellent sync, moderate power" instead of the C-E-G chime.

If you see `External audio NOT ready -- summary will use chime fallback` you
need to redo steps 1-3 (the QSPI either wasn't flashed or the blob got
corrupted). The device will play the chime safely in the meantime.
